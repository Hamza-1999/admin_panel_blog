...
export default function CreateBlog() {
  ...
  const handleClick = async () => {
    if (heading && slug) { 
      ...
      try {
        const response = await axios.post(`${Config.apiUrl}/admin/newBlog` , formData , {
          headers: {
  ...
  return (
    <>
      ...
      <Box sx={{ margin: '10px' }}>
        ...
        <Box>
          <Stack sx={{ padding: '13px 0px' , display : 'flex' , flexDirection : 'row' , justifyContent : 'space-between' }}>
            <TextField
              size="small"
              sx={{width : '65%'}}
              value={heading}
